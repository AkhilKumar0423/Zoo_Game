...
** Description: Sets dynamic arrays for exhibit for all animal of sixe 300,
** filling each element with NULL, since they're empty. Gives the player
** $100k to start with as well.
*******************************************************************************/
Zoo::Zoo() {
	exhibit = new Animal *[300];
...
void Zoo::deleteAnimalArrays() {
	...
	while (exhibit[i] != NULL and i < 100) {
		delete exhibit[i];
		i++;
	}

	i = 100;
	while (exhibit[i] != NULL and i < 200) {
		delete exhibit[i];
		i++;
	}

	i = 200;
	while (exhibit[i] != NULL and i < 300) {
		delete exhibit[i];
		i++;
	}
...
void Zoo::animalPurchaseMenu() {
	std::cout
		<< "You need to buy at least one animal out of three different species."
		<< std::endl;
	monkeyPurchaseMenu();
...
int Zoo::calculateFeedingCosts() {
	...
	while (exhibit[i] != NULL and i < 100) {
		totalCost += exhibit[i]->getBaseFoodCost();
		i++;
	}
...
int Zoo::calculatePayoff() {
	...
	while (exhibit[i] != NULL and i < 100) {
		totalPayoff += exhibit[i]->getPayoff();
		if (exhibit[i]->getAge() < 7) {
			totalPayoff += exhibit[i]->getPayoff();
		}
		i++;
	}
...
void Zoo::monkeyDies() {
	...
	if (exhibit[i - 1]->getAge() < 7) {
		std::cout << "Baby monkey got sick." << std::endl;
		if (bank >= (exhibit[i - 1]->getSick() * 2)) {
			makePayment(4800);
			std::cout << "Baby monkey has been cured and it took $4800 to cure "
						 "the disease."
					  << std::endl;
			std::cout << "Remaining balance is $" << getBankAccount()
					  << std::endl;
		} else {
			delete exhibit[i - 1];
			exhibit[i - 1] = NULL;
		...
		if (bank >= (exhibit[i - 1]->getSick())) {
			makePayment(2400);
			std::cout << "Monkey has been cured and it took $2400 to cure the "
						 "disease."
					  << std::endl;
			std::cout << "Remaining balance is $" << getBankAccount()
					  << std::endl;
...
void Zoo::seaotterDies() {
	...
	if (exhibit[i - 1]->getAge() < 7) {
		...
		if (bank >= (exhibit[i - 1]->getSick() * 2)) {
			makePayment(1600);
			std::cout << "Baby seaotter has been cured and it took $1600 to "
						 "cure the disease."
					  << std::endl;
			std::cout << "Remaining balance is $" << getBankAccount()
					  << std::endl;
		...
		if (bank >= (exhibit[i - 1]->getSick())) {
			makePayment(800);
			std::cout << "Seaotter has been cured and it took $800 to cure the "
						 "disease."
					  << std::endl;
			std::cout << "Remaining balance is $" << getBankAccount()
					  << std::endl;
...
void Zoo::meerkatDies() {
	...
	if (exhibit[i - 1]->getAge() < 7) {
		...
		if (bank >= (exhibit[i - 1]->getSick() * 2)) {
			makePayment(200);
			std::cout << "Baby meerkat has been cured and it took $200 to cure "
						 "the disease."
					  << std::endl;
			std::cout << "Remaining balance is $" << getBankAccount()
					  << std::endl;
		...
		if (bank >= (exhibit[i - 1]->getSick())) {
			makePayment(100);
			std::cout << "meerkat has been cured and it took $100 to cure the "
						 "disease."
					  << std::endl;
			std::cout << "Remaining balance is $" << getBankAccount()
					  << std::endl;
...
void Zoo::sickAnimal() {
	...
	switch (roll) {
	case 1:
		if (exhibit[0] == NULL) // If no monkeys in the array, give message.
		{
			std::cout << "A monkey sickness has spread, but there are no "
						 "monkeys left."
					  << std::endl;
			break;
		...
		break;
	case 2:
		if (exhibit[100] == NULL) // If no seaotters in the array, give message.
		{
			std::cout << "A seaotter sickness has spread, but there are no "
						 "seaotters left."
					  << std::endl;
			break;
		...
		break;
	case 3:
		if (exhibit[200] == NULL) // If no meerkats in the array, give message.
		{
			std::cout << "A meerkat sickness has spread, but there are no "
						 "meerkats left."
					  << std::endl;
			break;
...
void Zoo::boomInAttendance() {
	// For each monkey, get 500-1200 extra dollars.
	int i = 0;
	int totalmonkeys = 0;
	int bonus = 0;
...
void Zoo::babyBorn() {
	...
	switch (roll) {
	case 1:
		while (exhibit[i] != NULL and i < 100) // Check if any monkeys are at
											   // least 104 weeks (2years) old.
		{
			if (exhibit[i]->getAge() >= 104) // If they are, a baby is born.
			{
				count++;
			}
			i++;
